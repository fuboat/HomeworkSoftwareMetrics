                 ...
                 uint8_t kk, uint8_t nn)
{
    ...
    blocks = (block_t*) calloc(dd, sizeof(block_t));
    if (blocks == NULL)
    {
        return NULL;
    }

    dest = (uint8_t*) malloc(nn * sizeof(uint8_t));
    if (dest == NULL)
    {
        free(blocks);
        return NULL;
...
int main() {
    ...
    // blake2b
    auto digest = blake2b((const uint8_t*) input.c_str(), input.size(), NULL, 0, 64);
    // assert_bytes((const uint8_t*) input.c_str(), digest, 64);
